...
/// Persists only the minimum engagement state needed to show a quiet GitHub support action.
struct EngagementTracker {
    ...
    func shouldShowGitHubStarPrompt() -> Bool {
        guard defaults.integer(forKey: Key.successfulApplications) >= 1,
    ...
    /// The first successful application earns the small, non-blocking support action.
    @discardableResult
    ...
    func snoozePrompt() {
        ...
        defaults.set(now().addingTimeInterval(sevenDays), forKey: Key.promptSnoozedUntil)
    }
...
final class AppState {
    ...
    init(engagementDefaults: UserDefaults = .standard) {
        FontCatalog.registerStoredFonts()
        preferences = engagementDefaults
        engagementTracker = EngagementTracker(defaults: engagementDefaults)
        // English is deliberately the stable fallback. Older builds stored "system", which
        // could unexpectedly select another bundled localization even on an English Mac.
        appLanguage = AppLanguage(
            rawValue: engagementDefaults.string(forKey: LanguagePreference.languageKey) ?? "en"
        ) ?? .english
        showingLanguageChooser = !engagementDefaults.bool(
            forKey: LanguagePreference.completedFirstRunKey
        )
    }
    ...
    var appLanguage: AppLanguage = .english {
        didSet { preferences.set(appLanguage.rawValue, forKey: LanguagePreference.languageKey) }
    }
    var showingLanguageChooser = false

    var needsInitialLanguageChoice: Bool {
        !preferences.bool(forKey: LanguagePreference.completedFirstRunKey)
    }

    func confirmLanguageSelection() {
        preferences.set(true, forKey: LanguagePreference.completedFirstRunKey)
        showingLanguageChooser = false
    }
    ...
    private func recordSuccessfulStyleApplication() {
        showingGitHubStarPrompt = engagementTracker.recordSuccessfulStyleApplication()
    }
