...
struct FolderFill: Codable, Hashable {
    ...
    /// Whether imported artwork stays completely visible or covers the folder face.
    var imageContentMode: ImageContentMode = .fill

    init() {}

    /// Keep image fills from older style files intact when this newer setting is absent.
    init(from decoder: Decoder) throws {
        let container = try decoder.container(keyedBy: CodingKeys.self)
        kind = (try? container.decodeIfPresent(FillKind.self, forKey: .kind)) ?? .color
        ...
        imageContentMode = (try? container.decodeIfPresent(ImageContentMode.self,
                                                            forKey: .imageContentMode)) ?? .fill
    }
}
...
struct FolderStyle: Codable, Hashable, Identifiable {
    ...
    /// A font family installed on this Mac or imported into FolderForge. `nil` is the system font.
    ...
    init(from decoder: Decoder) throws {
        ...
        textFontName = (try? container.decodeIfPresent(String.self, forKey: .textFontName)) ?? nil
