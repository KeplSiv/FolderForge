...
private struct FillTab: View {
    ...
    private var imagePlacementControls: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            Picker("", selection: style.fill.imageContentMode) {
    ...
    private func loadFill(from url: URL) -> Bool {
        ...
        if isICNS {
            ...
            state.style.fill.fullIconData = nil
            state.style.fill.imageContentMode = .fit
            setImagePlacement(scale: 0.72)
        }
        ...
        case .symbol:
            ...
            Picker("Weight", selection: style.symbolWeight) {
                ForEach(FolderStyle.SymbolWeight.allCases) { weight in
                    Text(LocalizedStringKey(weight.title)).tag(weight)
                }
            }
            .controlSize(.small)
        ...
        case .text:
            ...
            LabeledContent("Font") {
                Button {
                    ...
                    HStack(spacing: 6) {
                        Text(state.style.textFontName ?? "System Default")
                            .font(state.style.textFontName.map { .custom($0, size: 11) }
                                ?? .system(size: 11))
                            .lineLimit(1)
            ...
            .sheet(isPresented: $showingFontPicker) {
                FontPickerSheet(selection: style.textFontName) { message in
                    state.toast = Toast(kind: .failure, message: "Couldn't import font",
                                        detail: message)
                }
            }
        ...
        if overlayKinds.contains(state.style.overlay.kind), state.style.overlay.kind != .none {
            ...
            Picker("", selection: style.finish) {
                ForEach(OverlayFinish.allCases) { finish in
                    Text(LocalizedStringKey(finish.title)).tag(finish)
                }
            }
            .labelsHidden()
            Text(LocalizedStringKey(state.style.finish.help))
                .font(.system(size: 10))
                .foregroundStyle(.tertiary)
                .fixedSize(horizontal: false, vertical: true)
