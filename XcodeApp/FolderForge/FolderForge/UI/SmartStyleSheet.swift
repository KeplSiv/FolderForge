    ...
    private var summaryRow: some View {
        HStack(spacing: 8) {
            ...
            Button { withAnimation(.easeOut(duration: 0.14)) { isExpanded.toggle() } } label: {
                HStack(spacing: 9) {
                    VStack(alignment: .leading, spacing: 1) {
                        Text(LocalizedStringKey(rule.kind.title))
                            .font(.system(size: 11, weight: .medium))
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 7) {
                ...
                Picker("Rule type", selection: kind) {
                    ForEach(SmartRuleKind.allCases) { kind in
                ...
                if rule.kind == .folderName || rule.kind == .relativePath {
                    ...
                    Picker("Match", selection: match) {
                        ForEach(FolderNameMatch.allCases) { match in
                            Text(LocalizedStringKey(match.title)).tag(match)
                        }
                    }
                    .frame(width: 108)
                }
            ...
            HStack(spacing: 7) {
                Text(LocalizedStringKey(rule.kind == .fileTypeMajority ? "Extensions" : "Pattern"))
                    .font(.system(size: 11))
                    .foregroundStyle(.secondary)
                    .frame(width: 74, alignment: .leading)
                ...
                Picker("Style source", selection: styleSource) {
                    ForEach(SmartRuleStyleSource.allCases) { source in
                        Text(LocalizedStringKey(
                            source == .custom && customPresets.isEmpty ? "My Styles (0)" : source.title
                        ))
                            .tag(source)
